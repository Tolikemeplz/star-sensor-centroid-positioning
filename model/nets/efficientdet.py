...
from model.nets.efficientnet import EfficientNet as EffNet
...
class EfficientNet(nn.Module):
    def __init__(self, bx, phi,in_channels=1,pretrained=False):
        ...
        model = EffNet.from_pretrained(f'efficientnet-b{bx}', pretrained, in_channels=in_channels)
        ...
        if 1 <= self.phi and self.phi <= 8:
            for i in range(4):
                attention_layer = nn.Sequential(
                    attention_block[self.phi - 1](dims[bx][i])
                )
    ...
    def forward(self, x):
        ...
        for idx, block in enumerate(self.model._blocks):
            ...
            if block._depthwise_conv.stride == [2, 2]:
                if 1 <= self.phi and self.phi <= 8:
                    if i>0:
                        last_x = self.attentions[i-1](last_x)
                    i=i+1
                    #print(i)
                # b=b+1
                # print(b)
...
def efficient(phi,pretrained=False,bx=0,  **kwargs):
    if pretrained:
        backbone_phi = [0, 1, 2, 3, 4, 5, 6, 7]
        return EfficientNet(backbone_phi[bx], phi, pretrained=pretrained,in_channels=3)
    else:
        backbone_phi = [0, 1, 2, 3, 4, 5, 6, 7]
        return EfficientNet(backbone_phi[bx], phi, pretrained=pretrained, in_channels=1)
